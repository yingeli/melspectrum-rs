...
fn main() -> Result<()> {
    ...
    println!(
        "Average time per iteration: {:.3} ms",
        duration.as_secs_f64() * 1000.0 / num_iterations as f64
    );
    println!(
        "Throughput: {:.2} iterations/second",
        num_iterations as f64 / duration.as_secs_f64()
    );
    println!("Audio processed: {:.1} seconds per iteration", 30.0);
    println!(
        "Real-time factor: {:.2}x",
        (30.0 * num_iterations as f64) / duration.as_secs_f64()
    );
